...
export const DataGridHeader = <T,>({ columns, sortConfig, filters, onSort, onFilter }: DataGridHeaderProps<T>) => {
                ...
                {columns.map((column) => {
                    ...
                    return (
                        <Th
                            ...
                            width={column.width}
                            aria-sort={
                                isSorted ? (direction === SortDirection.ASC ? 'ascending' : 'descending') : 'none'
                            }
                        >
                                    ...
                                    {column.sortable && (
                                        <SortIconContainer>
                                            {direction === SortDirection.ASC ? (
                                                ...
                                                <ArrowUpDown
                                                    size={14}
                                                    style={{ opacity: 1, color: theme?.colors?.muted }}
                                                />
                                            )}
                                        </SortIconContainer>
                                    )}
                                    ...
                                    >
                                        ...
                                        {isFiltering && (
                                            <FilterPopover ref={popoverRef} $rightAligned={isLastVisible}>
                                                <FilterPopoverTitle>Filter by {column.label}</FilterPopoverTitle>
                                                <FilterInputContainer>
                                                    <FilterIconWrapper>
